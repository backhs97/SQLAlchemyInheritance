...
def add_student_LetterGrade(sess):
    ...
    while not valid_input:
        min_satisfactory = input("Please enter the grade: ")
        if min_satisfactory in valid_grades:
            valid_input = True
        else:
            print("Invalid grade. Please enter a valid grade (A, B, C, D, F).")

    letter_grade = LetterGrade(section, student, min_satisfactory)
...
def delete_section(session: Session):
    ...
    n_enrollments = session.query(Enrollment).filter(Enrollment.sectionNumber == section.sectionNumber).count()
    if n_enrollments > 0:
...
def boilerplate(sess):
    ...
    major3: Major = Major(department, 'Chemical Engineering', 'Much closer to the cell')
    student1: Student = Student('Back', 'Jake', 'david.brown@gmail.com')
    student2: Student = Student('Jang', 'Jae', 'marydenni.brown@gmail.com')
    student3: Student = Student('Kim', 'Sam', 'disposable.bandit@gmail.com')
    student1.add_major(major1)
    student2.add_major(major1)
    student2.add_major(major2)
    student3.add_major(major3)
    ...
    section2: Section = Section(course2, 2, 'Spring', 2023, 'ECS', 415, 'MWF', time(8, 0, 0), 'David')
    sess.add(department)
    sess.add(course1)
    sess.add(course2)
    sess.add(major1)
